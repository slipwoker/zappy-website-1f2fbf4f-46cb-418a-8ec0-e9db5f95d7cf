    ...
    contactForm.addEventListener('submit', async function(e) {
      ...
      if (!window.zappyContactFormLoaded) {
        try {
          ...
          const zappyPayload = {
            websiteId: '1f2fbf4f-46cb-418a-8ec0-e9db5f95d7cf',
            name: name,
            email: email,
            subject: service ? `שירות: ${service}` : 'פנייה מטופס יצירת קשר',
            message: message,
            phone: phone || null
          };
          ...
          if (response.ok) {
            ...
            console.error('❌ Zappy: Failed to send email - HTTP', response.status);
          }
        } catch (error) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "עדי פרץ סוכנת נדל\"ן משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
